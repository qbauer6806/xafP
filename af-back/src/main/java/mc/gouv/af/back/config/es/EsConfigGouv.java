...
public class EsConfigGouv {
    ...
    public Client client() throws UnknownHostException {
        Settings esSettings = Settings.builder()
                .put(CLUSTER_NAME_PROPERTY, demGouvPropertiesResolver.getEsClusterName()).build();
        TransportClient client = new PreBuiltTransportClient(esSettings);
        // https://www.elastic.co/guide/en/elasticsearch/guide/current/_transport_client_versus_node_client.html
        return client.addTransportAddress(new InetSocketTransportAddress(
                InetAddress.getByName(demGouvPropertiesResolver.getEsHost()), demGouvPropertiesResolver.getEsPort()));
    }
