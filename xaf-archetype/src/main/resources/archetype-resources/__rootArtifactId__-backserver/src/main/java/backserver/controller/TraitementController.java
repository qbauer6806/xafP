...
import mc.gouv.xaf.back.service.MotifTemplateService;
import mc.gouv.xaf.back.util.AfBackUtils;
import mc.gouv.xaf.back.util.DemandesComplementsComparator;
...
public class TraitementController extends AbstractController {
    ...
    private static final String I18N_CREATION_DEMANDE_SUCCESS_CODE_MESSAGE = "message.success.creation.demande";
    ...
    public ModelAndView form(@PathVariable(value = "demandeId") Integer demandeId,
        ...
        // Charger certaines informations dans le formulaire si la demande est déjà en cours de traitement
        if (demande.getDernierStatut().getLibelle().equals(${artifactIdCamelCase}DemandeStatutEnum.EN_COURS_TRAIT.name())) {
            ...
            if (variables != null) {
                ...
                if (!StringUtils.isBlank(targetState)) {
                    // Conversion en DemandeStatutEnum pour s'assurer de l'existence de ce statut public
        ...
        if (statutPublicOuInterne.getName().equals(${artifactIdCamelCase}DemandeStatutEnum.EN_ATTENTE_COMPL.name())) {
            // Si on est en attente d'informations complémentaires ou en acceptée sous réserve, donner à la page le
            // icId
        ...
        // Définir pour la page, les actions disponibles et leurs statuts cibles associés, si on est à une étape de
        // choix
        ...
        // Vérification ou ajout à la main du motif dans la liste si celui-ci a été sélectionné mais ensuite
        // désactivé
        ...
        if ("fo".equals(origin)) {
            ...
            messages.add(messageSource.getMessage(I18N_CREATION_DEMANDE_SUCCESS_CODE_MESSAGE, null, Locale.FRENCH));
    ...
    public ModelAndView prendreEnCharge(@RequestParam(required = true) Integer pkDemande,
        ...
        DemandeHistoriqueDTO histo = histoService.prendreEnCharge(pkDemande,
                ${artifactIdCamelCase}DemandeStatutEnum.EN_COURS_TRAIT.name(),
    ...
    /**
     * Plus utilisé normalement Je le laisse pou le moment s'ils veulent revenir dessus.
     * 
     * @param traitementFormBean
     * @param pkDemande
     * @return
    ...
    public ModelAndView sauvegarder(@ModelAttribute("traitementFormBean") TraitementFormBean traitementFormBean,
        ...
        // Si l'agent qui sauvegarde n'est pas l'agent à qui est affectée la demande, effectuer une reprise en charge
    ...
    public ModelAndView dupliquer(@RequestParam(required = true) Integer pkDemande) throws Exception {
        ...
        try {
            demandeDupliquee = demandesService.cloneDemande(gouvPropertiesResolver.getDemarcheId(), pkDemande);
            ...
            demandesStatutsService.updateStatut(gouvPropertiesResolver.getDemarcheId(),
                    ...
                    demandeDupliquee.getAgentAffecteId(), null, null, "DUPLICATION");
