...
public class GouvBPMExpirationCheckDelegate implements JavaDelegate {
    ...
    public void execute(DelegateExecution execution) {
        ...
        LocalDate lastStatusDate = Instant.ofEpochMilli(demandeDto.getDernierStatut().getDate().getTime())
        ...
        LOGGER.info("pkDemande = {}, days = {}, numberOfDaysInt = {}", demandeDto.getPkDemandes(), days,
                numberOfDaysInt);

        if (days > numberOfDaysInt) {
            LOGGER.info("Demande expirée !");
            gouvBPM.setProcessBusinessVariable(demandeDto.getPkDemandes(),
                    GouvBPMProcessVariableTypeEnum.MC_EXPIRED.name(), true);
        }
