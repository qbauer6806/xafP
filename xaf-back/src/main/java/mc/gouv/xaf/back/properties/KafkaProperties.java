...
public class KafkaProperties {
    ...
    @Value("${mc.gouv.${application.name}.backapi.kafka.enabled:false}")
    private String kafkaEnabled;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.bootstrapserversconfig}")
    private String bootstrapServersConfig;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.ssl.enabled:false}")
    private String kafkaSSLEnabled;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.ssl.truststore.location}")
    private String truststoreLocation;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.ssl.truststore.password}")
    private String truststorePassword;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.ssl.keystore.location}")
    private String keystoreLocation;

    @Value("${mc.gouv.af.back.external.gichuni.kafka.ssl.keystore.password}")
    private String keystorePassword;

    @Value("${mc.gouv.${application.name}.backapi.kafka.producer.maxrequestsizeconfig:20971520}")
    private String maxRequestSizeConfig;

    @Value("${mc.gouv.${application.name}.backapi.kafka.consumer.fetchmaxbytes:20971520}")
    private String fetchMaxBytes;

    @Value("${mc.gouv.${application.name}.backapi.kafka.consumer.maxpartitionfetchbytes:20971520}")
