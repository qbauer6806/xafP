 ...
 * demande via un Job lancé depuis le BO. L'API n'a pas accès à ce code.
 *
 * @author qdeme
 */
@Service
...
public class GUKafkaDLTConsumer {
     ...
     * sur activation via Job depuis la page des Jobs du BO.<br> Il sert à copier les messages sur le topic initial afin
     * que l'API les traite une nouvelle fois.<br> Les messages sont acknowledged sur le DLT.
     */
    ...
    public void dltListen(ConsumerRecord<String, Object> consumerRecord, @Headers Map<String, String> header,
            Consumer<?, ?> consumer) {

        String topic = consumerRecord.topic();
        String key = consumerRecord.key();
        Object value = consumerRecord.value();
        LOGGER.info("Message reçu sur le DLT ({}, {}, {}, {}) : {}", topic, consumerRecord.partition(),
        ...
        LOGGER.info("Current offset : {}, initialLogEndOffset : {}", consumerRecord.offset(),
                initialEndOffsetsPerPartition.get(consumerRecord.partition()));

        String topicInitial = "gichuni-to-ts-" + gouvPropertiesResolver.getApplicationName();
        LOGGER.info(
                "Remise du message sur le topic initial ({}), sur la même partition ({}) et avec la même clé ({})...",
                topicInitial, consumerRecord.partition(), key);
        ...
        nbMessagesTraitesParPartition.put(consumerRecord.partition(),
                nbMessagesTraitesParPartition.get(consumerRecord.partition()) + 1);
        ...
        if (hasEverythingBeenRead()) {
            LOGGER.info(
                    "logEndOffset atteint sur toutes les partitions, arrêt du du KafkaListener gichuni-to-ts-consumer-dlt...");
            MessageListenerContainer listenerContainer = kafkaListenerEndpointRegistry.getListenerContainer(
                    "gichuni-to-ts-consumer-dlt");
    ...
    private Map<Integer, Integer> getLogEndOffsets(Consumer<?, ?> consumer) {
        List<KafkaMetric> me = (List<KafkaMetric>) consumer.metrics().values().stream()
                .filter(m -> "records-lead".equals(m.metricName().name())).toList();
        Map<Integer, Integer> map = new HashMap<>();
        for (KafkaMetric km : me) {
            for (String key : km.metricName().tags().keySet()) {
                if ("partition".equals(key)) {
                    map.put(Integer.parseInt(km.metricName().tags().get(key)),
                            Integer.parseInt(km.metricValue().toString().replace(".0", "")));
                }
            }
        }
        return map;
    ...
    public String traiterDLT() {
        ...
        LOGGER.info(
                "Démarrage du KafkaListener gichuni-to-ts-consumer-dlt puis attente jusqu'à fin de la redirection des messages...");
        ...
        try {
            ...
            while (jobOn) {
                ...
                if (nbSleep >= timeout && initialEndOffsetsPerPartition == null) {
                    LOGGER.info(
                            "Déjà {} secondes écoulées et initialEndOffsetsPerPartition non initialisé, il n'y a donc aucun message à traiter dans le DLT.",
                            timeout);
                    jobOn = false;
                }
            }
        ...
        if (nbMessagesTraites <= 1) {
            msg = nbMessagesTraites + " message du Dead Letter Topic a été remis sur le topic initial " + topicInitial
                    + " " + builder + ".";
        } else {
            msg = nbMessagesTraites + " messages du Dead Letter Topic ont été remis sur le topic initial "
                    + topicInitial + " " + builder + ".";
        }
