 ...
 * ou au contraire qui n'a pas pu être remis.
 *
 * @author qdeme
 */
@Component
...
public class GUKafkaProducerListener implements ProducerListener<String, String> {
    ...
    public void onSuccess(ProducerRecord<String, String> producerRecord, RecordMetadata recordMetadata) {
        ...
        if (topic.endsWith(".DLT")) {
            ...
            LOGGER.info("Message envoyé avec succès sur le DLT {} (key={}, partition={}, value={})", topic, key,
                    producerRecord.partition(), value);
            sendMailKafka(producerRecord, null, MAIL_TEMPLATE_KAFKA_DLT_OBJET, MAIL_TEMPLATE_KAFKA_DLT_CORPS);
        } else if (pkKafkaOutbox == null) {
            LOGGER.error(
                    "Message envoyé avec succès mais pkKafkaOutbox null ! Situation anormale, impossible de supprimer le message de l'outbox");
        } else {
    ...
    public void onError(ProducerRecord<String, String> producerRecord, Exception exception) {
        ...
        if (pkKafkaOutbox == null) {
            LOGGER.error(
                    "Erreur lors de l'envoi du message dans Kafka et pkKafkaOutbox null ! Situation anormale, impossible de mettre à jour son statut dans l'Outbox pour un retry",
                    exception);
            sendMailKafka(producerRecord, null, MAIL_TEMPLATE_KAFKA_ERREUR_ENVOI_OBJET,
                    MAIL_TEMPLATE_KAFKA_ERREUR_ENVOI_CORPS);
        } else {
            ...
            if (dto.getNbFailedAttempts() >= kafkaOutboxSchedulingConfig.getRetryNb()) {
                sendMailKafka(producerRecord, dto, MAIL_TEMPLATE_KAFKA_ERREUR_ENVOI_OBJET,
                        MAIL_TEMPLATE_KAFKA_ERREUR_ENVOI_CORPS);
            }
        }
    }
    ...
    private void sendMailKafka(ProducerRecord<String, String> producerRecord, KafkaOutboxDTO kafkaOutbox, String objet,
            String corps) {
        ...
        if (propertiesDTO.getValue() != null) {
            ...
            LOGGER.error(
                    "Erreur lors de l'envoi de l'e-mail : propriété XAF_ADRESSES_MAIL_SUPPORT_TECHNIQUE manquante !");
        }
    }
