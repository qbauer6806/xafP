...
public class RechercheUtils {
    ...
    private void doSearch(String search, String function, List<Path> roots, List<Predicate> predicatFTS, CriteriaBuilder cb) {
        for (Path root : roots) {
            predicatFTS.add(cb.isTrue(cb.function(
                    ...
                    root,
                    cb.function(
                            function, String.class, cb.literal(search)
                    )
            )));
        }
    }
