...
function checkPropertiesToUpdate(that) {
  ...
  if (propIndex === -1
      && (!equals(propLabel, rowData.label)
          || !equals(catId, rowData.categoryId) || enabled
          !== rowData.enabled)) {
    ...
    function (e) {
      ...
      if (this.name.startsWith("fichier")) {
        var idComplement = "complement" + this.name.replace(/\./g, '')
            + "Check";
        $("#" + idComplement).removeAttr("disabled").click().attr("disabled",
            true);
        var idFichierInterne = "fichierinterne" + this.name.replace(/\./g, '')
            + "Check";
  ...
  $.ajax({
    ...
    success: function (data) {
      $("#successMessage").data("message",
          "La configuration a été enregistrée avec succès").click();
    ...
    error: function (e) {
      $("#errorMessage").data("message",
          "Un problème est survenu lors de l'enregistrement de la configuration").click();
